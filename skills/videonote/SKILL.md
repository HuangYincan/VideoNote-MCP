...
2. **单视频一回合一个提交**。多集 / 多链接：每个 url 一个 **subagent**（提交 → 轮询 → 汇报），主 agent 汇总。不要在同一条消息里并行多个 `generate_note`。上限 `VIDEONOTE_MAX_WORKERS`（默认 3）。
3. **AGENT 自己写笔记**（用户明确要 `agent_direct` / 「你自己写」）：`prepare_note_material` → 轮询 → `get_task_transcript` + Read `frames` → 你写 Markdown。有 `comments_danmaku` 加「观众观点」一节（不捏造）。这是可选分支，不是默认。
