...
## 2026-09-11 — 共享逻辑解耦与文档完整性（未发版）

- **预检依赖**：平台识别与本地路径策略提取到轻量共享模块，预检不再导入完整 pipeline 或反向导入 MCP。MCP 显式传入目录授权，独立预检无需启动 DB/转写引擎。
- **转写读取**：CLI/MCP/Resource 共用 `task_artifacts.py`，统一规范缓存优先及损坏/无效容器回退，保留合法空转写；不改写原缓存。已知失败/运行中任务拒绝导出，UNKNOWN 旧任务仍可恢复导出但不视为成功；Resource 仍需 SUCCESS。
- **确定性导出**：默认目录按调用时配置读取，不再为路径导入 note/DB/引擎；相对输出目录返回绝对 URI。保留 CLI 任意显式导出目录与 MCP 数据目录门禁的区别，不放宽授权。
- **仓库维护**：README 中英文、架构/手册/索引、接手与贡献指南、VENDOR 和架构图同步；修正“模板已移除”与退役工具说明，贡献流程不要求个人记忆或私有 Skills。历史原始模板未改动，10 工具与现有 MCP 配置兼容。
- **验证**：新增 66 项回归，全量 **1263 passed, 1 skipped, 10 subtests passed**；Ruff F/I、shell、diff 检查通过。源码/wheel 的实际 stdio 增加预检/路径拒绝、转写回退/导出验证；wheel/sdist 含新模块，无 Skills/commands，26 个模板原件哈希不变。
- **范围与版本**：server 任务生命周期与 CLI 启动副作用保留，跨进程 TOCTOU 未纳入本次；未重新运行真实登录/媒体下载/ASR 或模板编译。版本不变（0.1.28），未发布。

## v0.2.1 — 2026-09-11

- **抖音登录与预检**：整合手机确认后的浏览器二次验证等待、登录态保留与多种抖音链接归一；预检复用原生 Cookie/msToken/ABogus 路径，避免把可恢复状态误报为必须重扫。
- **独立 MCP 与模板**：保持精确 10 个工具和现有 MCP 配置兼容；移除 Skills/commands 分发，MCP 自描述官方仓库与用法；恢复 26 个原始 LaTeX/Typst 模板及许可证，模板通过工具/Resource 独立发现、读取和复制。
...
## 2026-09-15 — Windows 导入停顿 / ffmpeg 卡死 / GPU 转写与显存释放（本地补丁，未发版）

> 本批改动**全部在本地工作区**，未提交、未发版。与上游逐字节一致性已不成立（8 文件）。

- **GPU 转写（本机启用）**：`is_cuda()` 原以 `torch.cuda.is_available()` 为唯一判据，但 whisper 推理走 ctranslate2、全程不碰 torch —— 判据错位。改为先问 ctranslate2（`get_cuda_device_count()` + cuBLAS 实际可加载的双级探测），torch 仅作兜底；新实现在所有组合下返回值均为旧实现的**超集**（只修正误判，绝不把 True 判成 False）。实测 8.88x（RTX 5060，large-v3-turbo，float16 vs CPU int8）。
...
- **导入停顿（本地规避）**：MCP 进程首次导入 faster_whisper（连带 av / numpy / ctranslate2）会停顿 48~455s（自愈、每进程一次、只在 MCP 启动的 server 里发生；普通进程 0/10）。新增 `VIDEONOTE_PREHEAT_TRANSCRIBER`（默认开）：在 `_pool` 创建前、主线程预热整条链。同一活跃期内 A/B：无预热 total=401.3s（FROZEN）vs 有预热 4.3s（ok）。**根因未落到实证**，此为工程规避；只预热 numpy 仅覆盖约 62%，必须整条链。
...
- **验证**：新增 `tests/test_gpu_idle_release.py`（26 用例 + 13 subtest，全绿）。全量套件改动前后失败集合逐条相同（Windows 环境固有失败，与本批无关）。
