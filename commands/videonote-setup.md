...
- 清理单任务：**`cleanup(task_id, include_note?, dry_run=True)`** 先看占用的文件 → `dry_run=False` 再删。
- 全局清理：**`cleanup(dry_run=True)`** 先预览 → 确认后 `cleanup(include_config?, include_models?)`（默认保留配置与模型）。
